...
export async function POST(
  ...
  if (!(file instanceof File)) return NextResponse.json({ error: "Missing receipt field" }, { status: 400 });
  ...
  const path = `car-repairs/${userId}/${id}/${randomUUID()}.${ext}`;
  ...
  if (upload.error) {
    return NextResponse.json({ error: "Upload failed", details: upload.error.message }, { status: 502 });
  }
  ...
  return NextResponse.json({ receiptUrl: updated.receiptUrl, receiptPath: updated.receiptPath });
}
...
export async function DELETE(
  ...
  await supabaseAdmin.storage.from(IMAGE_BUCKET).remove([repair.receiptPath]);
  await prisma.carRepair.update({ where: { id }, data: { receiptUrl: null, receiptPath: null } });
